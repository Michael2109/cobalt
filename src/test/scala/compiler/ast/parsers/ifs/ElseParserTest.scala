...
import compiler.ast.blocks.ifs.ElseBlock
...
class ElseParserTest() extends FunSuite with BeforeAndAfter {
  ...
  test("Block creation test") {
    for (line <- lines) {
      ...
      assert(parseable.head.parse(null, new Tokenizer(line)).isInstanceOf[ElseBlock])
    }
  }
