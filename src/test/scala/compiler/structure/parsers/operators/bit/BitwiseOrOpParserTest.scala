...
import compiler.structure.blocks.operators.bit.BitwiseOrOpBlock
...
class BitwiseOrOpParserTest() extends FunSuite with BeforeAndAfter {
  ...
  test("Block creation test") {
    for (line <- lines) {
      ...
      assert(parseable.head.parse(null, new Tokenizer(line)).isInstanceOf[BitwiseOrOpBlock])
    }
  }
