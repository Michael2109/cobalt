...
import compiler.structure.blocks.operators.DivideOpBlock
...
class DivideOpParserTest() extends FunSuite with BeforeAndAfter {
  ...
  test("Block creation test") {
    for (line <- lines) {
      ...
      assert(parseable.head.parse(null, new Tokenizer(line)).isInstanceOf[DivideOpBlock])
    }
  }
