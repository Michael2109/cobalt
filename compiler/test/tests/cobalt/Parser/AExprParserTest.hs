...
{-
testAExprParserExpr :: Test
testAExprParserExpr = do

    let codeAdd = "x + 100"
    let testAdd = TestCase $ assertEqual codeAdd
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeAdd) of
                           Left  e -> error $ show e
                           Right x -> x)
    ...
    let testSubtract = TestCase $ assertEqual codeSubtract
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeSubtract) of
                           Left  e -> error $ show e
                           Right x -> x)
    ...
    let testMultiply = TestCase $ assertEqual codeMultiply
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeMultiply) of
                           Left  e -> error $ show e
                           Right x -> x)
    ...
    let testDivide = TestCase $ assertEqual codeDivide
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeDivide) of
                           Left  e -> error $ show e
                           Right x -> x)
    ...
    let testMixed = TestCase $ assertEqual codeMixed
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeMixed) of
                           Left  e -> error $ show e
                           Right x -> x)
    ...
    let testNegative = TestCase $ assertEqual codeNegative
                       (AExprContainer $ IntConst 10)
                       (case (parse expressionParser' "" codeNegative) of
                           Left  e -> error $ show e
                           Right x -> x)
