...
instance IRGen Expr where
    genIR (ABinary aBinOp aExpr1 aExpr2) symbolTable currentState = EmptyIR
    genIR (Add) symbolTable currentState = EmptyIR
    genIR (And) symbolTable currentState = AndIR 
    ...
    genIR (ClosingParenthesis) currentState symbolTable = EmptyIR
    ...
    genIR (Divide) symbolTable currentState = EmptyIR
    ...
    genIR (Function name annotations params returnType static exprs) st cs = FunctionIR  (genIR name st cs) (maybeExprToMaybeIRNode annotations st cs) (exprArrToIRArray params st cs) (genIR returnType st cs) static (exprArrToIRArray exprs st cs)
    ...
    genIR (IntConst i) symbolTable currentState = EmptyIR
    genIR (LessEqual) symbolTable currentState = LessEqualIR 
    genIR (Less) symbolTable currentState = LessIR 
    ...
    genIR (Multiply) symbolTable currentState = EmptyIR
    genIR (Neg aExpr) symbolTable currentState = EmptyIR
    ...
    genIR (OpeningParenthesis) currentState symbolTable = EmptyIR
    genIR (Or) st cs = OrIR 
    ...
    genIR (Parenthesis aExpr) st cs = EmptyIR
