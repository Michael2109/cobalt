...
import compiler.ast.blocks.operators.MultiplyOpBlock
...
class MultiplyOpParserTest() extends FunSuite with BeforeAndAfter {
  ...
  test("Block creation test") {
    for (line <- lines) {
      ...
      assert(parseable.head.parse(null, new Tokenizer(line)).isInstanceOf[MultiplyOpBlock])
    }
  }
