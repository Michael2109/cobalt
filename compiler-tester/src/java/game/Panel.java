...
private boolean alive;private boolean aliveBool=false;private boolean alive(){ if(!aliveBool){aliveBool=true;alive=true;}return alive;}private void alive_(final boolean alive){this.aliveBool=true;this.alive=alive;}private Game game;public Game game(){return game;}public void game_(Game game){ this.game=game;}public Panel(Game game){this.game=game;thread().start();} public void update(){
x_(x() + dx());
y_(y() + dy());
if(x() < 0 || x() > 750){
dx_(dx() * -1);}
