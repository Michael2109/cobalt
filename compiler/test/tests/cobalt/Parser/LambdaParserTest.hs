...
testLambdaParser = do
    ...
    let testInline = testParseSuccess codeInline (Lambda [Field {fieldName = Name "x", fieldType = Nothing, fieldInit = Nothing}] (ExprAssignment (Identifier (Name "x")))) statementParser

    let codeInlineType = "fun (x:Int) -> x"
    let testInlineType = testParseSuccess codeInlineType (Lambda [Field {fieldName = Name "x", fieldType = Just (TypeRef (RefLocal (Name "Int"))), fieldInit = Nothing}] (ExprAssignment (Identifier (Name "x")))) statementParser

    let codeInlineMultiple = "fun (x,y,z) -> x"
    let testInlineMultiple = testParseSuccess codeInlineMultiple (Lambda [Field {fieldName = Name "x", fieldType = Nothing, fieldInit = Nothing},Field {fieldName = Name "y", fieldType = Nothing, fieldInit = Nothing},Field {fieldName = Name "z", fieldType = Nothing, fieldInit = Nothing}] (ExprAssignment (Identifier (Name "x")))) statementParser
    ...
    let testDoBlock = testParseSuccess codeDoBlock (Lambda [Field {fieldName = Name "x", fieldType = Nothing, fieldInit = Nothing}] (StmtAssignment $ BlockStmt [ExprAsStmt (Identifier (Name "x")),ExprAsStmt (Identifier (Name "y"))])) statementParser

    let codeDoBlockType = unlines [ "fun (x:Int) -> do"
                              , "    x"
                              , "    y"
                              ]
    let testDoBlockType = testParseSuccess codeDoBlockType (Lambda [Field {fieldName = Name "x", fieldType = Just (TypeRef (RefLocal (Name "Int"))), fieldInit = Nothing}] (StmtAssignment $ BlockStmt [ExprAsStmt (Identifier (Name "x")),ExprAsStmt (Identifier (Name "y"))])) statementParser
